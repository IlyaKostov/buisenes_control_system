...
class InviteService(BaseService):
    ...
    @transaction_mode
    async def create_invite(self, account: AccountModel, background_tasks: BackgroundTasks) -> InviteModel:
        ...
        background_tasks.add_task(sending_email_with_invite_code, account.email, invite_token)

        return invite

    @transaction_mode
    async def update_invite(self, account: AccountModel, background_tasks: BackgroundTasks) -> InviteModel:
        ...
        background_tasks.add_task(sending_email_with_invite_code, account.email, new_token)
