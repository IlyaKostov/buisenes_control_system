...
class StructAdmService(BaseService):
    ...
    @transaction_mode
    async def delete_department(self, department_id: uuid.UUID, admin: AccountModel):
        department = await self.uow.struct_adm.get_by_query_one_or_none(id=department_id)
    ...
    async def check_manager(self, manager_id: uuid.UUID, admin: AccountModel):
    ...
    async def check_department(self, data: CreateStructAdm, admin: AccountModel):
